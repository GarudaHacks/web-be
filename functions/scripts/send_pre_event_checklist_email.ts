 ...
 *
 * Each successful send stamps the user's doc with reminderEmailSent=true and
 * reminderEmailSentAt (server timestamp); subsequent runs skip anyone already
 * marked, so it's safe to re-run for stragglers without double-emailing.
 *
 * Dry-run by default - only counts/lists recipients, sends nothing.
 *
 * To run (from functions/):
 *   npx tsx scripts/send_pre_event_checklist_email.ts                    # dry-run
 *   npx tsx scripts/send_pre_event_checklist_email.ts --send --test-email you@x.com  # smoke test
 *   npx tsx scripts/send_pre_event_checklist_email.ts --send             # send to everyone not yet sent
 *   npx tsx scripts/send_pre_event_checklist_email.ts --send --limit 25  # first 25 only
 *   npx tsx scripts/send_pre_event_checklist_email.ts --send --force     # resend to everyone, ignoring reminderEmailSent
 *
 * Env overrides:
...
const SUBJECT = "Pre-Event Checklist Update: RSVP, GH App, Discord, Participant Info Session, Workshops";
 ...
 * status, skipping anyone already marked reminderEmailSent unless `force` is set.
 */
async function getRecipients(force: boolean): Promise<Recipient[]> {
  ...
  for (const doc of snap.docs) {
    ...
    if (!force && data.reminderEmailSent === true) {
      alreadySent++;
      continue;
    }
  ...
  if (alreadySent > 0) {
    console.log(`Skipping ${alreadySent} user(s) already marked reminderEmailSent (pass --force to resend).`);
  }
  return recipients;
}
...
async function main() {
  ...
  const html = fs.readFileSync(path.resolve(__dirname, "../src/templates/emailReminder.html"), "utf-8");
      ...
      if (uid) {
        try {
          await db.collection("users").doc(uid).set(
            { reminderEmailSentAt: FieldValue.serverTimestamp(), reminderEmailSent: true },
            { merge: true }
          );
        } catch (err) {
          console.warn(`  ! sent to ${email} but failed to record reminderEmailSentAt: ${(err as Error).message}`);
        }
      }
    } catch (err) {
  ...
  const logPath = path.join(logDir, `pre_event_checklist_${Date.now()}.json`);
