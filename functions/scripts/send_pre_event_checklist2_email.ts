/**
 * Send the "Pre-Event Checklist Update: RSVP, GH App, Discord, Participant
 * Info Session" reminder email to every user whose application status is
 * "accepted" or "confirmed rsvp".
 *
 ...
 *
 * Each successful send stamps the user's doc with reminderEmail2Sent=true and
 * reminderEmail2SentAt (server timestamp); subsequent runs skip anyone already
 * marked, so it's safe to re-run for stragglers without double-emailing. This is
 * a separate flag from the first checklist email's reminderEmailSent, so this
 * script sends independently of who already got checklist 1.
 *
 * Dry-run by default - only counts/lists recipients, sends nothing.
 *
 * To run (from functions/):
 *   npx tsx scripts/send_pre_event_checklist2_email.ts                    # dry-run
 *   npx tsx scripts/send_pre_event_checklist2_email.ts --send --test-email you@x.com  # smoke test
 *   npx tsx scripts/send_pre_event_checklist2_email.ts --send             # send to everyone not yet sent
 *   npx tsx scripts/send_pre_event_checklist2_email.ts --send --limit 25  # first 25 only
 *   npx tsx scripts/send_pre_event_checklist2_email.ts --send --force     # resend to everyone, ignoring reminderEmailSent
 *
 * Env overrides:
...
const app = admin.initializeApp(
  ...
  "send-pre-event-checklist-email"
);
const db = app.firestore();

const SUBJECT = "Pre-Event Checklist Update: Info Session Recording, Participant Handbook, Workshops, RSVP, App, MAC Address, Discord";
 ...
 * status, skipping anyone already marked reminderEmail2Sent unless `force` is set.
 */
async function getRecipients(force: boolean): Promise<Recipient[]> {
  ...
  for (const doc of snap.docs) {
    ...
    if (!force && data.reminderEmail2Sent === true) {
      alreadySent++;
      continue;
    }
  ...
  if (alreadySent > 0) {
    console.log(`Skipping ${alreadySent} user(s) already marked reminderEmail2Sent (pass --force to resend).`);
  }
  return recipients;
}
...
async function main() {
  ...
  const html = fs.readFileSync(path.resolve(__dirname, "../src/templates/emailReminder2.html"), "utf-8");
      ...
      if (uid) {
        try {
          await db.collection("users").doc(uid).set(
            { reminderEmail2SentAt: FieldValue.serverTimestamp(), reminderEmail2Sent: true },
            { merge: true }
          );
        } catch (err) {
          console.warn(`  ! sent to ${email} but failed to record reminderEmail2SentAt: ${(err as Error).message}`);
        }
      }
    } catch (err) {
  ...
  const logPath = path.join(logDir, `pre_event_checklist2_${Date.now()}.json`);
